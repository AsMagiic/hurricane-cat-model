...
4. ~~**Industry output formats**~~ — DONE: ~~OED exposure format (Location + Account,
   ...
   DONE (Step 4.3); ~~reinstatements on XoL layers~~ DONE (Step 4.4).
...
- 2026-06-13 — Step 4.4 DoD CLOSED — Finite reinstatements with annual aggregate tracking and programme cost implemented. n_reinstatements=1 per layer (market-standard for cat XoL); annual aggregate limit = occ_limit × (1 + n). Tower config migrated from model_v3.yaml to config/reinsurance.yaml (adds occ_limit rename from limit, n_reinstatements, reinstatement_premium_pct, loading_factor). model_config.py: load_reinsurance_cfg() added; _materialize_layers docstring updated; test_model_config.py line 103 updated (tree.reinsurance.layers → tree.summary.return_periods, since reinsurance removed from model_v3.yaml). model/reinsurance.py rewritten: _occ_recovery() (vectorized uncapped occurrence), apply_xol_unlimited() (regression baseline), apply_xol_finite() (year-by-year aggregate cap via sorted EventId order); programme metrics per layer (EL, base premium TECHNICAL 15% loading, E[reinstatement premium], hit freq, reinstatement-use freq); finite-vs-unlimited AEP-net comparison table; module now imports load_reinsurance_cfg() instead of load_model_cfg(). GROSS BIT-IDENTICAL: AAL gross 9.151M, all OEP/AEP gross metrics unchanged. NET BASELINE UNCHANGED BY FINDING (not by design): Layer 1 reinstatement exhaustion occurs in 51/100k years (0.05%); finite vs unlimited delta = 0.00M at ALL return periods (1-in-25 through 1-in-1000). At n=1, the programme's aggregate design (2× occ_limit per layer) is more than sufficient for this portfolio's occurrence frequency. Net production baseline confirmed unchanged: AAL 7.709M, OEP-100 60.0M, OEP-250 60.0M, AEP-100 70.32M, AEP-250 90.61M. Reinstatement premium pro-rata to amount; pro-rata-temporis is v4. Programme cost TECHNICAL (flat EL loading 15%): total E[base premium] = $1.659M, E[reinst premium] = $0.0002M (negligible, < 0.01% of base). tests/test_reinsurance.py: 10 tests (3 hand-example, 1 gross-unchanged, 1 unlimited-regression, 1 agg-cap, 1 order-independence, 1 direction, 2 reinstatement-premium). 259/259 tests pass (10 new).
