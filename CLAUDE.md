...
- 2026-06-12 — 3.0b open question (UNRESOLVED, tagged to 3.0c): the WPR residual raises mean Δp by exp(σ²/2)=1.031 over the deterministic line (Jensen bias; verified: 90.5% of the −0.259M AAL drop comes from this systematic Rmax reduction, not sub-physical near-misses). Whether this is correct depends on whether the OLS log-fit coefficient `a` in calibration/wind_pressure.py estimates the MEDIAN or the MEAN of conditional Δp. If `a` is the MEDIAN (the OLS fit of ln(Δp) gives the median in level space), applying exp(ε) is correct — E[Δp]=a·V^b·exp(σ²/2) recovers the true mean. If `a` was fitted to target the MEAN (e.g. by back-transforming with a bias-correction term), then exp(ε) double-counts the Jensen term and the residual should instead use exp(ε − σ²/2) (mean-preserving scatter). MUST resolve before wpr=on ships — check the fitting procedure in calibration/wind_pressure.py. Do NOT change the centering until that review is complete.
